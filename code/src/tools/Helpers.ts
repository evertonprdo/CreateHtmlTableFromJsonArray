...
export abstract class Helpers {
     ...
     */
    static makeMyOwnDomTableDataObject(html_query:string): DataObjectType {
        ...
        function pushKeys(ths: NodeList): string[] {
            ...
            let sufix = 1;
            ths.forEach(th => {               
                let x = Helpers.stringToSlug((th as HTMLTableCellElement).innerText, true);
                if (keys.includes(x)) {
                    x += '-d-' + sufix;
                    sufix++;
                }
